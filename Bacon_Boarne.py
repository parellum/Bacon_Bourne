AGENT_NAME                  =       "Bacon_Boarne"
TEAM_NAME                   =       "Cogal_Jacques"

    
# These are algorithm parameters. You need to change this
DELTA                       =      -0.00025              #   How much should the epsilon shift by in every game
LR                          =      0.10          #   This is the learning rate. High learning rate means faster learning"                     
GAMMA                       =      0.90              #   gamma = discount factor. High gamma value means focus on future rewards "
STARTING_EPSILON            =      1

#These are the reward parameters. You need to change this. Refer to the guide to know what each means in detail
DESTROY_SOFTBLOCK           =      1000   #+     
DO_DAMAGE                   =      10000  #++
EARN_AMMO                   =      250   #+
EXIST_PENALTY               =      -1  #-
COLLIDE_WITH_WALLS          =      -100  #---- 
NO_DESTRUCTION              =      -100  #--                 
NO_KILL                     =      -100  #--        
PLACING_BOMBS_WITH_NO_AMMO  =      -100  #----
BOMB_IN_RANGE               =      -50  #--
PLACING_BOMB_ON_TOP_OF_BOMB =      -50  #--
DAMAGE_ITSELF               =      -2000 # Want AI to value life. Otherwise will end life quicker to avoid exist_penalty 
TAKE_DAMAGE                 =      -2000 # Want AI to value life. Otherwise will end life quicker to avoid exist_penalty 
...
class Agent:
    ...
    def next_move(self, game_state, player_state):
        ...
        if(done):
            ...
            try:
                trackWin=open("Tracker4.txt","r")
            except:
                trackWin=open("Tracker4.txt","w")
                trackWin.close()
                trackWin=open("Tracker4.txt","r")
            winRecord=trackWin.readlines()
            trackWin.close()
            trackWin=open("Tracker4.txt","a")
            if winRecord==[]:
                latestRecord=[0,0]
            else:
                latestRecord=winRecord.pop()
                latestRecord=latestRecord.split(" ")
                latestRecord=list(map(int,latestRecord))
            if(player_state.hp==0):
                latestRecord[1]+=1
                latestRecord=" ".join(list(map(str,latestRecord)))+"\n"
                trackWin.write(latestRecord)
                trackWin.close()
                print("LOST")
            else:
                latestRecord[0]+=1
